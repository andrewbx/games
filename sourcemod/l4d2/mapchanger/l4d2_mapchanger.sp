...
new Handle:g_hCheckEmptyServer = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	hKVSettings=CreateKeyValues("ForceMissionChangerSettings");
  	BuildPath(Path_SM, FMC_FileSettings, 128, "data/l4d2_mapchanger.txt");
...
public OnMapStart()
{
	...
	if(GetConVarInt(Allowed) == 1)
	{
		...
		if (StrContains(current_map, "c1m") != -1)
			SetConVarInt(CheckRoundCounter, 3);
			...
			SetConVarInt(CheckRoundCounter, 2);
                        ...
                        SetConVarInt(CheckRoundCounter, 4);
                else if (StrContains(current_map, "c9m") != -1)
                        SetConVarInt(CheckRoundCounter, 1);
                else if (StrContains(current_map, "c10m") != -1)
                        SetConVarInt(CheckRoundCounter, 4);
                else if (StrContains(current_map, "c11m") != -1)
                        SetConVarInt(CheckRoundCounter, 4);
                else if (StrContains(current_map, "c12m") != -1)
                        SetConVarInt(CheckRoundCounter, 4);
                else if (StrContains(current_map, "c13m") != -1)
                        SetConVarInt(CheckRoundCounter, 3);
...
public Action:Timer_CheckEmptyServer(Handle:hTimer)
{
	...
	//LogMessage("[+] T_CES: Checking for empty server. (Repeat:60s)");
...
public Action:Event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	g_hCheckEmptyServer = CreateTimer(30.0, Timer_CheckEmptyServer, _, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
}
...
public Action:Event_RoundEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (g_hCheckEmptyServer != INVALID_HANDLE)
	{
                CloseHandle(g_hCheckEmptyServer);
                g_hCheckEmptyServer = INVALID_HANDLE;
	}
		...
		if (StrEqual(next_mission_force, "none") != true)
		{
			if (StrContains(next_mission_force, "_airport0") != -1)
				...
				announce_map = "The Parish";
                                ...
                                announce_map = "No Mercy";
                        else if (StrContains(next_mission_force, "c9m") != -1)
                                announce_map = "Crash Course";
                        else if (StrContains(next_mission_force, "c10m") != -1)
                                announce_map = "Death Toll";
                        else if (StrContains(next_mission_force, "c11m") != -1)
                                announce_map = "Dead Air";
                        else if (StrContains(next_mission_force, "c12m") != -1)
                                announce_map = "Blood Harvest";
			else if (StrContains(next_mission_force, "c13m") != -1)
                                announce_map = "Cold Stream";
...
public Action:TimerChDelayVS(Handle:timer)
{
	...
	if (StrContains(current_map, "l4d_garage02_lots") != -1)
        ...
        //if (StrContains(current_map, "c8m5_rooftop") != -1)
	//{
	//	Sub_FlushClients("Flushing connectivity after 4 rotations.");
	//	ServerCommand("map %s %s", next_mission_force, DEFAULT_GAMEMODE);
	//}
	//else
	//{
	ForceChangeLevel(next_mission_force, current_map);
...
bool:IsValidMode()
{
        ...
        if
        (
                StrContains(GameMode, "versus") != -1 ||
                StrContains(GameMode, "teamversus") != -1 ||
                StrContains(GameMode, "mutation12") != -1
        )
                return true;
